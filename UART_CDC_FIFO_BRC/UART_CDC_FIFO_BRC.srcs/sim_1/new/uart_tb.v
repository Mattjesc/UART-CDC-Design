...
module uart_tb;
    ...
    initial begin
        $monitor("Time: %0t | TX_CLK: %b | RX_CLK: %b | START: %b | TXIN: %0h | TX: %b | RX: %b | RXOUT: %0h | TXDONE: %b | RXDONE: %b | bitDone_tx: %b | bitDone_rx: %b | TX_FIFO_FULL: %b | TX_FIFO_EMPTY: %b | TX_FIFO_WR_EN: %b | TX_FIFO_RD_EN: %b | RX_FIFO_FULL: %b | RX_FIFO_EMPTY: %b | RX_FIFO_WR_EN: %b | RX_FIFO_RD_EN: %b | BAUD_RATE: %0d", 
                 $time, tx_clk, rx_clk, start, txin, txrx, txrx, rxout, txdone, rxdone, bitDone_tx, bitDone_rx, tx_fifo_full, tx_fifo_empty, tx_fifo_wr_en, tx_fifo_rd_en, rx_fifo_full, rx_fifo_empty, rx_fifo_wr_en, rx_fifo_rd_en, baud_rate);
    end

    initial begin
        start = 0;
        txin = 0;
        baud_rate = 9600; // Initial baud rate
        ...
        baud_rate = 19200; // Change baud rate to 19200
        #100; // Wait for the new baud rate to take effect
        ...
        baud_rate = 38400; // Change baud rate to 38400
        #100; // Wait for the new baud rate to take effect
